...
// basementstudio shader-lab composition, with the "Text" source layer changed
// to render the name. WebGL only mounts client-side (mount guard below).
const config = {
  layers: [
    {
      blendMode: "normal",
    ...
    },
    {
      ...
      params: {
        ...
        animate: true,
        motionAmount: 0.81,
        // Motion slowed (was 2) so the sub-10fps shader reads as a gentle
        // drift instead of a stutter — full CRT look kept, just calmer.
        motionSpeed: 0.85,
        falloff: 3.5,
        tonemapMode: "totos",
        glowStrength: 0,
...
export default function ShaderBackground() {
  ...
  useEffect(() => {
    // Respect reduced motion: skip the animated WebGL shader entirely.
    const mq = window.matchMedia("(prefers-reduced-motion: reduce)");
    if (mq.matches) return;
    setMounted(true);
  }, []);

  // Renders nothing until client mount (WebGL can't SSR) and nothing at all
  // for reduced-motion users — the scrim/background behind it stays.
